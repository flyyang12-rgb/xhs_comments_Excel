...
<p align="center">
  <strong>v1.2.3</strong> · Windows 64 位 · Chrome · 便携版
</p>
...
2. **启动本机服务**：双击 `启动采集工具.bat`，按窗口内的中文分色步骤操作。插件路径会复制到剪贴板；看到「准备就绪」后保持服务窗口打开。重复启动时也会显示完整引导。
...
> 从 1.2.1 / 1.2.2 升级：覆盖原插件目录，在扩展管理页点击「重新加载」，确认插件版本为 **1.2.3**。本机服务仍为 1.2.1，无需重启。从更早版本升级时，请关闭旧服务并启动包内服务。

1.2.3 修复立即停止时丢失已取得评论、二级回复计数遗漏及重复翻页问题。评论逐页保留，停止或请求异常时自动尝试下载结果；分页没有进展时停止请求。保留 Cookie 补读、进度日志和中文分色启动引导。新增异常场景回归测试已通过；此前用户反馈已成功请求搜索和一级评论，本次停止及异常翻页修复通过模拟接口验证。
