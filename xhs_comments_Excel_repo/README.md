# 小红书笔记与评论采集

这是面向运营同事的小红书 Chrome 采集插件，可按关键词搜索，也可粘贴一条指定笔记链接，采集公开笔记、一级评论和二级评论并导出 Excel。日常使用不需要编程基础。当前版本：`v1.1.0`。

## Chrome 插件（推荐）

### 使用前准备

- Windows 10/11
- Chrome
- 可正常访问网络（首次运行会自动安装所需组件）
- 已登录小红书的 Chrome 会话

### 使用步骤

1. 下载项目 ZIP，并完整解压到一个固定文件夹。
2. 双击 `启动采集工具.bat`。首次运行会自动检查并安装运行环境，请不要关闭窗口。
3. 脚本会打开 Chrome 扩展管理页和插件文件夹，并自动复制插件路径。
4. 在 Chrome 开启“开发者模式”，点击“加载已解压的扩展程序”，选择 `xhs_chrome_extension`。
5. 登录小红书，打开插件，选择“关键词采集”或“笔记链接”。关键词模式可设置排序、类型和发布时间；链接模式直接粘贴一条笔记链接。
6. 采集期间保持启动窗口打开；完成后导出 `notes_raw.xlsx` 和 `comments_raw.xlsx`。

更详细的插件说明见 [`xhs_chrome_extension/README.md`](xhs_chrome_extension/README.md)。

## 文档

- [快速使用指南](docs/快速使用指南.md)
- [AI 应用案例申报模板](docs/AI应用案例申报模板.md)
- [Python 旧版工作流](docs/Python旧版工作流.md)
- [文档索引](docs/README.md)

旧版 Python 脚本仅作为开发维护资料保留，普通使用者无需运行。

## 数据与账号安全

- `xhs_cookies.txt`、Excel/CSV 采集结果、依赖目录和本地 Node 运行时已被 `.gitignore` 排除。
- 插件只在本机读取当前 Chrome 登录态，并通过 `127.0.0.1:18765` 调用本地签名服务。
- 请遵守目标平台规则、适用法律和合理的请求频率；验证码或安全限制出现时应停止采集。
